...
import { Satellite, Loader2, Phone } from "lucide-react";
...
const Auth = () => {
  ...
  const handleSendOtp = async () => {
    if (!validatePhone(phone)) {
      toast.error("Please enter a valid phone number with country code (e.g., +233123456789)");
      return;
    }
      ...
      <Card className="w-full max-w-md p-8 shadow-elevated">
        ...
        <div className="flex gap-2 mb-4">
          <Button
            ...
            className={`flex-1 ${authMethod === "email" ? "bg-gradient-ocean" : ""}`}
            onClick={() => {
              setAuthMethod("email");
              setOtpSent(false);
            }}
          ...
          <Button
            ...
            className={`flex-1 ${authMethod === "phone" ? "bg-gradient-ocean" : ""}`}
            onClick={() => {
              setAuthMethod("phone");
              setOtpSent(false);
            }}
