      ...
      .on(
        ...
        (payload) => {
          ...
          if (newAlert.user_id === user.id) {
            ...
            toast.warning(`⚠️ New ${newAlert.hazard_type} alert: ${newAlert.region_name}`, {
              duration: 8000,
            });
          }
        }
                    ...
                    <Card key={region} className="p-4 space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-2 text-xs">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="font-medium">{latest.temperature_c?.toFixed(1) ?? "—"}°C</span>
                        </div>
                        <div className="flex items-center gap-1.5">
                          <CloudRain className="h-3 w-3 text-blue-500" />
                          <span className="text-muted-foreground">Rain:</span>
                          <span className="font-medium">{latest.rainfall_mm?.toFixed(1) ?? "—"} mm</span>
                        </div>
                        <div className="flex items-center gap-1.5">
                          <Droplets className="h-3 w-3 text-cyan-500" />
                          <span className="text-muted-foreground">Soil:</span>
                          <span className="font-medium">{latest.soil_moisture?.toFixed(2) ?? "—"}</span>
                        </div>
                        <div className="flex items-center gap-1.5">
                          <Wind className="h-3 w-3 text-purple-500" />
                          <span className="text-muted-foreground">Wind:</span>
                          <span className="font-medium">{latest.wind_speed_kmh?.toFixed(1) ?? "—"} km/h</span>
                        </div>
                      </div>
